    ...
    const fetchMatches = async () => {
      ...
      try {
        const res = await axios.get<Match[]>(
          `http://localhost:8080/api/basketball/matches`
        );
        setMatches(res.data);
      } catch (err) {
      ...
      <div className="p-4 max-w bg-gray-300 m-10 mx-auto">
        ...
        {filteredLiveMatches.slice(0, liveLimit).map((match) => (
          ...
          >
            ...
            <button
              ...
              className="text-white bg-blue-500 px-3 py-1 text-sm rounded"
            >
              Bet Now
            </button>
          </div>
        ...
        {filteredUpcomingMatches.slice(0, upcomingLimit).map((match) => (
          ...
          >
            ...
            <button
              onClick={() => navigate(`match/${match.id}`)}
              className="text-white bg-green-500 px-3 py-1 text-sm rounded"
            >
              Pre-Bet
            </button>
          </div>
